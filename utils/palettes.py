...
class palette:
    ...
    def expand(self, color=None,N=1,method="unidirectional",param="h"):
        if color is None:
            color = choice(self.primary_colors)
    ...
    def expand_to_N(self, size=5):
        ...
        to_add = size-len(self.primary_colors)
        # Check
        if to_add <= 0:
            return 
        ...
        for i in range(len(self.primary_colors)):
            if i+1 == len(self.primary_colors): add_amount = to_add # last color gets all colors
            ...
            # add to palette
            self.expand(color=self.primary_colors[i],N=add_amount,method=method,param=param)
    def preview(self,verbose=False):
        print("Primary colors:")
        for i in self.primary_colors: i.preview(verbose=verbose)
        print("Extra colors:")
...
def random_N_palette(N=5,preview=False,base=None,scheme=None,weighted=False,preferential=False):
